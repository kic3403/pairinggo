...
기획·설계 문서는 docs/ (00~18). 데이터 운영은 docs/11, 앱 점검 결과·남은 과제는 docs/12·15, **최신 최적화·보완 진단은 docs/18**(2026-09-13), 경쟁 서비스(데일리샷·캐치테이블) 분석과 적용안은 docs/19.
...
- 회원 추천 페어링(회원픽, docs/13): `member_picks` → 같은 조합 `MEMBER_PICK_MIN`(2)명이면 `pairings` src 'user' 자동 생성(`lib/member-picks.ts`). 닉네임만 노출, 글 140자·링크 금지, 사진 1장(Storage `member-picks`), 카탈로그에 없는 이름은 `/admin/picks` 검수 후. 카탈로그 캐시는 15초마다 version을 확인한다(다른 프로세스의 발행 반영).
...
## 데이터
- 원본: packages/shared/data/pairings.json (전통주 217 · 음식 110 · 페어링 1,790 — 2026-09-13 라인업 확장 +99종 §5-4, 백경증류소 +7종·한증류소 +3종 §5-5, docs/11). DB가 원본이고 JSON은 `db:export`로 내보낸 번들.
