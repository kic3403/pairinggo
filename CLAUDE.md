...
- **식당 예약 + 페어링GO 파트너 앱**(2026-09-18, docs/21): 사장님은 `apps/partner`(PWA, Vercel 별도 프로젝트, 서명 쿠키 `pgo_partner`)에서 가입 신청 → `/admin/partners` 승인 → 영업시간·정원 정하고 예약 받기 켬. 예약은 **즉시 확정**(수락 단계 없음), 정원 판정은 DB 함수 `reserve()`(매장 설정 행 잠금), 전이는 `reservation_transition()` — 규칙은 `shared/reservation/`(한국 시간 기준). 손님은 `/reserve/[카카오id]`(로그인 + 휴대폰 문자 인증 + 예약마다 매장 제공 동의), `/my/reservations`(방문 1시간 전까지 취소). 매장 정보 수정은 `place_info`(source partner)에 바로 반영 + `merchant_changes` 이력(운영자 되돌리기, 운영자 전용 값은 유지). 알림은 `packages/server/notify`(웹 푸시 + 솔라피 알림톡, 저장 뒤 발송·실패는 기록만), 아침 크론 `/api/cron/reservations`가 당일 안내·보관 기한 정리. 솔라피 키 전에는 운영에서 예약 불가(개발은 인증번호를 서버 로그에). 손님 번호는 방문 90일 뒤 파트너 화면에서 가린다. **매장 메뉴판**(2026-09-19): 파트너 `/store`에서 메뉴판 사진 → Claude가 메뉴(음식명·설명·가격)·술(이름·용량·도수·가격)을 읽어 표에 더함(없는 칸은 빈칸, `shared/menu-items.ts`, `place_info.menu_items·drink_items` 0024, 읽기 본체 `packages/server/menu-read.ts`, 매장당 하루 20번) → 식당 카드·예약 화면 "메뉴판 보기"(`MenuBoard`). **식당 검색** `/places` + `/api/v1/places/search`(검색할 때만 카카오 호출, 주변 결과에 그 이름이 없으면 전국에서 찾아 앞에) + 술·음식 검색 결과 화면의 "‘검색어’ 식당" 칸(`SearchPlaces`, 이름·키워드 검색일 때만, `lite=1`로 구글 평점 생략·5곳). 식당 결과는 이름이 맞는 곳 먼저(`shared/places.ts sortByNameMatch` — 카카오는 메뉴명·태그까지 봐서 이름이 다른 곳도 준다). **검색창 자동완성**(`SearchBox`): 술·음식·종류/양조장은 `/api/v1/suggest`(shared search 그대로 — 부분 일치·입력 중 자모·초성, 외부 호출 없음, 0.12초), 식당은 두 글자 이상·0.4초 멈췄을 때만 `places/search?lite=1` → 이름 맞는 곳 3곳(`nameMatchedOrAll`), ↑↓ Enter(한글 조합 중 Enter 무시), 카드 목록은 `PlaceList`·붙이기는 `lib/place-enrich.ts`로 맛집 API와 공용. 레이트리밋은 `rateLimit(req, n, scope)` — 라우트마다 scope를 나눈다.
