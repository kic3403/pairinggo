...
- 운영자 식당 정보(2026-09-17 사용자 결정): 콜키지·룸·메뉴·취급 술은 어떤 공개 API에도 없고 네이버 지도·캐치테이블 화면 수집은 약관 위반 → **운영자가 전화·방문으로 확인해 `/admin/places`에 직접 입력**(카카오에서 식당 찾기 → 콜키지·룸·주차 + 메모, 취급 전통주·대표 메뉴는 카탈로그에서 선택). 표 `place_info`(0021, 카카오 장소 id 기준, source operator|partner — 제휴 식당 직접 입력도 같은 표), 규칙 `shared/place-info.ts`(`cleanPlaceInfo`·`placeChips`·`verifiedFirst`), 서버 `lib/place-info.ts`. 식당 검색 API가 매번 붙여 **확인된 식당을 목록 맨 앞**에, 이름 옆 칩(운영자 확인 = 남색, 구글 = 회색)과 "운영자 확인 2026.09 · 전통주 … · 메뉴 …" 상자로 보여 준다(주차는 운영자 값이 구글 값을 덮음). 식당 목록 API는 10분 캐시라 반영에 최대 10분. 회원 제보는 아직 없음(붙이면 운영자 값 아래 순위).
