...
describe("메뉴판 구분(2026-09-27)", () => {
  it("음료 구분은 남고 음식은 기본값(키 없음)", () => {
    const rows = cleanMenuItems([{ name: "석류탕", price: 18000 }, { name: "오미자차", price: 5000, section: "beverage" }, { name: "콜라", section: "weird" }, { name: "공깃밥", section: "other" }]);
    expect(rows[0].section).toBeUndefined();
    expect(rows[1].section).toBe("beverage");
    expect(rows[2].section).toBeUndefined();
    expect(rows[3].section).toBe("other");
  });
  it("손님 메뉴판 — 음식·주류(주종별)·음료", () => {
    const g = groupMenuBoard(
      [{ name: "석류탕", desc: "", price: 18000 }, { name: "오미자차", desc: "", price: 5000, section: "beverage" }, { name: "공깃밥", desc: "", price: 1000, section: "other" }],
    ...
    expect(g.other.map((x) => x.name)).toEqual(["공깃밥"]);
