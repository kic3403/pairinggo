...
import { formatBizNo, formatMobile, isManualPlaceId, MERCHANT_STATUS_LABEL } from "@pairinggo/shared";
    ...
    <>
      ...
      {rows.length === 0 ? <div className="card muted">아직 신청이 없어요.</div> : rows.map((m) => (
        <div className="card" key={m.id}>
          ...
          <PartnerActions id={m.id} status={m.status} manual={isManualPlaceId(m.kakaoPlaceId)} name={m.name} address={m.address} />
        </div>
      ))}
