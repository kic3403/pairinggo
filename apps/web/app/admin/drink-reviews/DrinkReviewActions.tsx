...
export default function DrinkReviewActions({ id, status }: { id: number; status: "active" | "hidden" }) {
  const [busy, setBusy] = useState(false);
  async function run(action: "hide" | "restore" | "delete") {
    const reason = action === "hide" ? window.prompt("숨김 사유(운영 기록)") ?? "" : "";
    if (action === "delete" && !confirm("이 평가를 지울까요? 되돌릴 수 없어요.")) return;
    setBusy(true);
    const r = await fetch("/admin/api/drink-reviews", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action, reason }) });
