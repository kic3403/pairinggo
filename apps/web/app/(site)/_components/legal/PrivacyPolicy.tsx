...
export const COLLECTION_ROWS: { when: string; items: string; purpose: string }[] = [
  ...
  { when: "서비스를 이용할 때", items: "저장한 전통주·음식·음식점, 먹어봤어요 평가, 술 평가(별점·한 줄), 회원 추천 글·사진·하트, 검색어, 술 추가 요청(요청한 이름·메모), 라벨 사진에서 읽은 술 이름(사진 자체는 AI가 읽은 뒤 저장하지 않습니다), 알림을 켠 기기의 알림 주소와 알림 설정(주간 소식·활동 소식), 화면 방문·링크 클릭 기록, 로그인 시각·실패 횟수", purpose: "저장·평가·추천 기능 제공, 페어링 통계와 서비스 개선, 부정 로그인 방지, 이용자가 켠 웹 푸시 알림(주간 소식·활동 소식) 발송" },
