...
const LABEL: Record<Tab, string> = { all: "전체", expert: "전문가픽", public: "대중픽", profile: "맛 분석" };
...
export default function PickTabs({ counts, children }: { counts: Record<PickKey, number>; children: ReactNode }) {
  ...
  const total = counts.expert + counts.public + counts.profile;
  const tabs: Tab[] = ["all", "expert", "public", "profile"];
  return (
