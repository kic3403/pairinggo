...
export async function POST(req: Request) {
  ...
  const rows = parsed.data.events.map((e) => {
    ...
    return { name: e.n, props, session_id: typeof sid === "string" ? sid.slice(0, 40) : null, client_ts: new Date(Math.min(e.t, now + 60_000)).toISOString() };
  });
  ...
  const logs = rows.filter((r) => SEARCH_KINDS.has(r.name) && typeof r.props.q === "string" && (r.props.q as string).trim()).map((r) => {
    ...
    return { query_text: q, query_norm: normalize(q), kind: r.name, matched_type: r.name === "search_intent" ? "intent" : mt, matched_id: mid, session_id: r.session_id };
  });
