/**
 * GET /api/account/consent → { needed } — 로그인 회원이 약관 동의를 (다시) 해야 하는지. SavedProvider가 화면마다 확인해 /profile(가입 마무리)로 보낸다.
 * 페이지 로드 때 불리므로 auth() 대신 getToken()(세션 쿠키를 다시 쓰지 않는다 — 첫 화면 로그아웃과 경합 방지).
 */
import { NextResponse } from "next/server";
import { getToken } from "next-auth/jwt";
import { consentNeeded } from "@/lib/account";
...
export async function GET(req: Request) {
  ...
  try { return NextResponse.json({ needed: await consentNeeded(uid), loggedIn: true }, { headers: NO_STORE }); }
