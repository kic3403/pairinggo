 ...
 */
import { cleanPlaceInfo, isEmptyPlaceInfo, type Place, type PlaceInfo } from "@pairinggo/shared";
import { db } from "./db";
...
type Row = {
  ...
  drink_ids: string[]; food_ids: string[]; menu_note: string; memo: string; source: PlaceInfo["source"]; verified_at: string | null; updated_by: string | null; updated_at: string;
};
export type PlaceInfoRow = { kakaoId: string; name: string; address: string; phone: string | null; placeUrl: string | null; memo: string; updatedBy: string | null; updatedAt: string; info: PlaceInfo };
...
const toInfo = (r: Row): PlaceInfo => ({
  ...
  drinks: r.drink_ids ?? [], foods: r.food_ids ?? [], menuNote: r.menu_note, source: r.source, verifiedAt: r.verified_at,
});
const toRow = (r: Row): PlaceInfoRow => ({ kakaoId: r.kakao_id, name: r.name, address: r.address ?? "", phone: r.phone, placeUrl: r.place_url, memo: r.memo, updatedBy: r.updated_by, updatedAt: r.updated_at, info: toInfo(r) });
...
export type PlaceInfoInput = { place: { id: string; name: string; address?: string; phone?: string | null; lat?: number; lng?: number; placeUrl?: string | null }; info: Record<string, unknown>; memo?: string; updatedBy?: string };
...
export async function savePlaceInfo(input: PlaceInfoInput): Promise<PlaceInfoRow> {
  ...
  if (isEmptyPlaceInfo(info)) throw new Error("적은 내용이 없어요 — 주차·콜키지·룸·전통주·메뉴 중 하나는 적어 주세요");
  ...
  const row = {
    ...
    drink_ids: info.drinks, food_ids: info.foods, menu_note: info.menuNote, memo: String(input.memo ?? "").slice(0, 300), source: info.source,
