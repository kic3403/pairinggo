 ...
 */
import { cleanContactPhone, cleanPlaceInfo, isEmptyPlaceInfo, type Place, type PlaceInfo } from "@pairinggo/shared";
import { db } from "./db";
...
type Row = {
  ...
  drink_ids: string[]; food_ids: string[]; drink_names: string[]; menu_names: string[]; contact_phone: string; naver_url: string | null; menu_note: string; memo: string; source: PlaceInfo["source"]; verified_at: string | null; updated_by: string | null; updated_at: string;
};
...
const toInfo = (r: Row): PlaceInfo => ({
  ...
  drinks: r.drink_ids ?? [], drinkNames: r.drink_names ?? [], foods: r.food_ids ?? [], menuNames: r.menu_names ?? [], menuNote: r.menu_note, naverUrl: r.naver_url ?? null, source: r.source, verifiedAt: r.verified_at,
});
